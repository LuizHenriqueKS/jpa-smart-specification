...
public class NotLikeJpaSpecificationFieldPredicateBuilder implements JpaSpecificationFieldPredicateBuilder {

    @Override
    public Set<String> getPatterns() {
        return Set.of("NotLike");
    }
    ...
    public Predicate build(JpaSpecificationArgs args) {
        ...
        if (value instanceof String) {
            return args.getCriteriaBuilder().notLike(path, (String) value);
        } else {
            Expression<String> exp = (Expression<String>) value;
            return args.getCriteriaBuilder().notLike(path, exp);
        }
    }
